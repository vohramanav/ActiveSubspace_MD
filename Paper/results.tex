...
The joint posterior of ($\alpha$,$\lambda$,$\gamma$) was computed on a 3D Cartesian grid containing
10$\times$10$\times$10 samples. For the purpose of illustration, the joint posterior was projected on
2D plans: $\alpha$-$\lambda$, $\alpha$-$\gamma$, and $\lambda$-$\gamma$ passing through the
maximum a posteriori (MAP) point that corresponds to the highest value of the posterior probability,
in each case. The co-ordinates of the MAP point were found to be 
($\alpha$ = 1.98, $\lambda$ = 23.10, $\gamma$ = 1.08)
The 2D contour plots are provided in Figure~\ref{fig:like}.
Experimental observation of 245~W/m/K for the
bulk thermal conductivity of silicon at 300~K~($\kappa_{\tiny{\mbox{E}}}$)~\cite{Volz:1999},
and a $\sigma$ value, 0.15$\ast$$\kappa_{\tiny{\mbox{E}}}$, were used to compute the 
joint posterior.
%
\begin{figure}[htbp]
 \begin{center}
...
%  \end{tabular}
\caption{Contour plots of the joint posteriors, ($\alpha$,$\lambda$), ($\alpha$,$\gamma$), and ($\lambda$,$
\gamma$) obtained by projecting the 3-dimensional joint posterior of ($\alpha$,$\lambda$,$\gamma$)
on a 2D plane passing through the MAP point (highlighted in each case).}
\label{fig:like}
\end{center}
\end{figure}
%
Note that the posteriors were evaluated using a single data point for 
...
It is encouraging to note that the joint posterior of $\alpha$ and $\gamma$,
...
Legendre polynomial basis~(see Figure~11 in~\cite{Vohra:2018a}). 
