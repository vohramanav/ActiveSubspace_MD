...
NEMD predictions~($\kappa_{\tiny{\mbox{MD}}}^c$) corrected for physics-based model error, 
...
physics-based model error~($\varepsilon_\text{\tiny{MD}}$) using~\eqref{eq:like1}~and~\eqref{eq:like2}.
...
Physics-based model error~($\varepsilon_\text{\tiny{MD}}$) was estimated using a polynomial chaos
...
The joint posteriors: ($\alpha$,$\lambda$), ($\alpha$,$\gamma$), and ($\lambda$,$\gamma$)
 were evaluated  on a 2D Cartesian grid containing
40$\times$40 samples, and are shown
using contour plots in Figure~\ref{fig:like}. Experimental value of 245~W/m/K for the
...
Marginal posteriors of  $\alpha$, $\lambda$ and $\gamma$ are also
...
%  \end{tabular}
\caption{Joint posteriors of ($\alpha$,$\lambda$), ($\alpha$,$\gamma$), and ($\lambda$,$\gamma$),
estimated using Eq.~\ref{eq:like} is plotted
on a (40$\times$40) 2D Cartesian grid. The maximum likelihood estimate (MLE) is also highlighted in 
each case. Bottom row: Marginal posteriors for $\alpha$, $\lambda$ and $\gamma$.}
\label{fig:like}
\end{center}
\end{figure}
%
Note that the joint posteriors were evaluated using a single data point for 
