...
error~($\varepsilon_\text{\tiny{ME}}$) using~\eqref{eq:kc}.

The surrogate error $\varepsilon_\text{\tiny{SE}}$ in this case is a normally distributed random
variable with mean 0. The variance was estimated using the discrepancy between the surrogate, $\mathcal{Y}$
and the function $\tilde{\mathcal{Y}}$ plotted in Figure~\ref{fig:casfig2}~(right).
