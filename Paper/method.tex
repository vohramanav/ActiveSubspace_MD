...
the gradients required for evaluating $\hat{\mat{C}}$ in~\eqref{eq:chat}. 
Instead, it involves a regression-based local linear approximation of the model output as
discussed in~\cite{Constantine:2015}. In this work, however, we implement the
...
The sequence of steps for computing the active subspace using the gradient-free approach is outlined
...
\bigskip
%
Note that the same sequence of steps is followed when using the gradient-based approach for computing the
